...
const EXERCISES = [
  ...
  { 
    ...
    instructions: [
      ...
      'Mantenha o peito erguido e olhar ao frente',
  ...
  { 
    ...
    instructions: [
      ...
      'Ideal: sair às 6h–7h30 para evitar o calor forte de Pernambuco',
  ...
  { 
    ...
    instructions: [
      ...
      'Rotação de pescoço: lentamente, gire a cabeça fazendo um arco de orelha a orelha (não force para trás). 5 cada lado.'
    ]
  },
  { 
  ...
  { 
    ...
    instructions: [
      ...
      'Respire livremente',
      'Este exercício é baseado em técnicas de liberação de tensão muscular (TRE)',
      'Benefício imediato: libera tensão acumulada nos músculos do pescoço, ombros e costas'
    ]
  },
];

export function AtividadeFisica() {
  const [currentDate, setCurrentDate] = useState(new Date());
  const [showHistory, setShowHistory] = useState(false);
  const [expandedId, setExpandedId] = useState<string | null>(null);
  
  const { physicalActivities, updatePhysicalActivity } = useData();

  const currentFormattedDate = format(currentDate, 'yyyy-MM-dd');
  const currentRecord = physicalActivities?.find(a => a.id === currentFormattedDate);
  ...
  const handleToggle = async (id: string, e: React.MouseEvent) => {
    ...
    await updatePhysicalActivity(currentFormattedDate, newCompleted);
  };
  ...
  const getWeekStats = () => {
    ...
    let count = 0;
    (physicalActivities || []).forEach(record => {
      try {
  ...
  const renderGroup = (groupName: string, icon: any, colorClass: string) => {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
           <div>
              <h2 className="text-xl font-bold mb-1 tracking-tight text-white flex items-center gap-2">
                <Dumbbell className="w-5 h-5 text-primary" /> Atividade Física & Bem-estar
              </h2>
              <p className="text-gray-400 text-sm">Exercício melhora memória, concentração e reduz o estresse. Rotinas rápidas para fazer em casa.</p>
           </div>
           <div className="flex flex-col items-end shrink-0 gap-2">
              ...
              <button onClick={() => setShowHistory(!showHistory)} className="text-xs text-primary hover:text-white transition-colors flex items-center gap-1 font-bold">
                <CalendarDays className="w-3 h-3" /> {showHistory ? 'Voltar para Hoje' : 'Ver Hitórico 30 dias'}
              </button>
           </div>
        </div>
      </div>
      ...
      {showHistory ? (
        <div className="bg-card rounded-xl border border-border p-5">
           ...
           <div className="space-y-3">
              {Array.from({length: 30}).map((_, i) => {
                 ...
                 const rec = physicalActivities?.find(a => a.id === dateStr);
                 const exCount = (rec?.completedIds || []).length;
                 if (exCount === 0 && i > 6) return null; // hide empty old days to declutter
        ...
        <>
          ...
          <div className="flex items-center justify-between mb-6 bg-surface p-3 rounded-xl border border-border">
            <button 
              onClick={prevDay}
              className="p-1.5 hover:bg-white/5 rounded text-gray-400 font-bold uppercase text-[10px] tracking-widest transition-colors flex items-center gap-1"
            >
            ...
            <button 
              onClick={nextDay}
              disabled={isToday}
              className="p-1.5 hover:bg-white/5 rounded text-primary font-bold uppercase text-[10px] tracking-widest transition-colors flex items-center gap-1 disabled:opacity-30 disabled:text-gray-500"
            >
